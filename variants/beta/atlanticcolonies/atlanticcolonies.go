...
func AtlanticColoniesGraph() *graph.Graph {
	return graph.New().
		...
		// Bermuda Triangle
		Prov("bet").Conn("his", godip.Sea).Conn("lee", godip.Sea).Conn("mia", godip.Sea).Conn("beu", godip.Sea).Conn("ese", godip.Sea).Conn("geo", godip.Sea).Conn("flo", godip.Sea).Conn("bah", godip.Sea).Conn("cub", godip.Sea).Conn("jam", godip.Sea).Conn("his", godip.Sea).Flag(godip.Sea).
		// Scotland
		...
		// Nova Scotia
		Prov("nov").Conn("now", godip.Sea).Conn("now", godip.Sea).Conn("iro", godip.Coast...).Conn("nee", godip.Coast...).Conn("ese", godip.Sea).Flag(godip.Coast...).SC(godip.Neutral).
		// Florida
		Prov("flo").Conn("geo", godip.Coast...).Conn("gom", godip.Sea).Conn("bah", godip.Coast...).Conn("bet", godip.Sea).Flag(godip.Coast...).
		// Detroit
		...
		// Southern Atlantic
		Prov("soa").Conn("sao", godip.Sea).Conn("coa", godip.Sea).Conn("soo", godip.Sea).Conn("sea", godip.Sea).Conn("gre", godip.Sea).Conn("swa", godip.Sea).Flag(godip.Sea).
		// Newfoundland
		Prov("nef").Conn("now", godip.Sea).Conn("ese", godip.Sea).Conn("wea", godip.Sea).Conn("now", godip.Sea).Conn("now", godip.Sea).Flag(godip.Coast...).SC(godip.Neutral).
		// Ontario
		...
		// Gulf of Mexico
		Prov("gom").Conn("flo", godip.Sea).Conn("geo", godip.Sea).Conn("neo", godip.Sea).Conn("saf", godip.Sea).Conn("mex", godip.Sea).Conn("mer", godip.Sea).Conn("wec", godip.Sea).Conn("cub", godip.Sea).Conn("bah", godip.Sea).Flag(godip.Sea).
		// Cayenne
		Prov("cay").Conn("caa", godip.Sea).Conn("sur", godip.Coast...).Conn("goy", godip.Land).Conn("eem", godip.Coast...).Flag(godip.Coast...).SC(France).
		// Grain Coast
		Prov("gra").Conn("sen", godip.Land).Conn("gor", godip.Coast...).Conn("gre", godip.Sea).Conn("gol", godip.Coast...).Conn("nig", godip.Land).Flag(godip.Coast...).
		// Windward Islands
		...
		Prov("now").Conn("hud", godip.Sea).Conn("mon", godip.Sea).Conn("mon", godip.Sea).Conn("mon", godip.Sea).Conn("iro", godip.Sea).Conn("nov", godip.Sea).Conn("nov", godip.Sea).Conn("ese", godip.Sea).Conn("nef", godip.Sea).Conn("nef", godip.Sea).Conn("nef", godip.Sea).Conn("wea", godip.Sea).Conn("ote", godip.Sea).Flag(godip.Sea).
		// Bermuda
		Prov("beu").Conn("ese", godip.Sea).Conn("bet", godip.Sea).Conn("mia", godip.Sea).Conn("wea", godip.Sea).Flag(godip.Coast...).SC(godip.Neutral).
		// Eastern Seaboard
		Prov("ese").Conn("beu", godip.Sea).Conn("wea", godip.Sea).Conn("nef", godip.Sea).Conn("now", godip.Sea).Conn("nov", godip.Sea).Conn("nee", godip.Sea).Conn("pen", godip.Sea).Conn("geo", godip.Sea).Conn("bet", godip.Sea).Flag(godip.Sea).
		// St. Louis
		Prov("stl").Conn("app", godip.Land).Conn("det", godip.Land).Conn("upp", godip.Land).Conn("lou", godip.Land).Conn("neo", godip.Land).Flag(godip.Land).SC(France).
		// Gibraltar
		Prov("gib").Conn("and", godip.Coast...).Conn("str", godip.Sea).Conn("val", godip.Coast...).Flag(godip.Coast...).SC(Spain).
		// Merida
		Prov("mer").Conn("sie", godip.Land).Conn("bei", godip.Coast...).Conn("wec", godip.Sea).Conn("gom", godip.Sea).Conn("mex", godip.Coast...).Flag(godip.Coast...).SC(Spain).
		// New Granada
		...
		// Pennsylvania
		Prov("pen").Conn("geo", godip.Coast...).Conn("ese", godip.Sea).Conn("nee", godip.Coast...).Conn("iro", godip.Land).Conn("app", godip.Land).Flag(godip.Coast...).
		// Maranhao
		...
		// West Sahara
		Prov("wsa").Conn("mis", godip.Land).Conn("som", godip.Land).Conn("cai", godip.Coast...).Conn("cse", godip.Sea).Conn("cav", godip.Coast...).Conn("sen", godip.Land).Conn("nig", godip.Land).Flag(godip.Coast...).
		// Nigrita
		...
		// Santa Fe
		Prov("saf").Conn("mex", godip.Coast...).Conn("gom", godip.Sea).Conn("neo", godip.Coast...).Conn("lou", godip.Land).Conn("cal", godip.Land).Conn("sie", godip.Land).Flag(godip.Coast...).SC(godip.Neutral).
		// Lima
		Prov("lim").Conn("sat", godip.Land).Conn("saa", godip.Land).Conn("bog", godip.Land).Conn("neg", godip.Land).Flag(godip.Land).SC(Spain).
		// Ireland
		Prov("ire").Conn("iri", godip.Sea).Conn("ote", godip.Sea).Conn("iri", godip.Sea).Flag(godip.Coast...).
		// Cape of Good Hope
		...
		// Namib Desert
		Prov("nam").Conn("kon", godip.Coast...).Conn("ang", godip.Coast...).Conn("sea", godip.Sea).Conn("soo", godip.Sea).Conn("cog", godip.Coast...).Conn("eaa", godip.Land).Flag(godip.Coast...).
		// Valencia
		...
		// English Channel
		Prov("eng").Conn("eur", godip.Sea).Conn("bre", godip.Sea).Conn("par", godip.Sea).Conn("ote", godip.Sea).Conn("lon", godip.Sea).Conn("bri", godip.Sea).Conn("iri", godip.Sea).Flag(godip.Sea).
		// Cape Verde
		Prov("cav").Conn("sen", godip.Land).Conn("wsa", godip.Coast...).Conn("cse", godip.Sea).Conn("mia", godip.Sea).Conn("caa", godip.Sea).Conn("swa", godip.Sea).Conn("gre", godip.Sea).Conn("gor", godip.Coast...).Flag(godip.Coast...).SC(godip.Neutral).
		// Western Caribbean
		...
		// Andalucia
		Prov("and").Conn("cas", godip.Coast...).Conn("por", godip.Coast...).Conn("eur", godip.Sea).Conn("str", godip.Sea).Conn("gib", godip.Coast...).Conn("val", godip.Coast...).Flag(godip.Coast...).
		// Catalan
		Prov("cat").Conn("bae", godip.Sea).Conn("guo", godip.Sea).Conn("mss", godip.Coast...).Conn("bor", godip.Land).Conn("cas", godip.Land).Conn("val", godip.Coast...).Flag(godip.Coast...).
		// Senegal
		Prov("sen").Conn("gra", godip.Land).Conn("nig", godip.Land).Conn("wsa", godip.Land).Conn("cav", godip.Land).Conn("gor", godip.Land).Flag(godip.Land).SC(godip.Neutral).
		// Rio de Janeiro
		...
		// Marseilles
		Prov("mss").Conn("cat", godip.Coast...).Conn("guo", godip.Sea).Conn("par", godip.Land).Conn("bor", godip.Land).Flag(godip.Coast...).SC(France).
		// Louisiana
		Prov("lou").Conn("cal", godip.Land).Conn("saf", godip.Land).Conn("neo", godip.Land).Conn("stl", godip.Land).Conn("upp", godip.Land).Conn("wrl", godip.Land).Conn("ore", godip.Land).Flag(godip.Land).
		// Baeleric Sea
		...
		// Bahamas
		Prov("bah").Conn("flo", godip.Coast...).Conn("gom", godip.Sea).Conn("cub", godip.Coast...).Conn("bet", godip.Sea).Flag(godip.Coast...).SC(godip.Neutral).
		// Buenos Aires
		...
		// Surinam
		Prov("sur").Conn("trn", godip.Coast...).Conn("cca", godip.Land).Conn("bog", godip.Land).Conn("cay", godip.Coast...).Conn("caa", godip.Sea).Flag(godip.Coast...).SC(godip.Neutral).
		// East Rupert's Land
		...
		// Cuba
		Prov("cub").Conn("wec", godip.Sea).Conn("jam", godip.Coast...).Conn("bet", godip.Sea).Conn("bah", godip.Coast...).Conn("gom", godip.Sea).Flag(godip.Coast...).
		// Tripoli
		...
		// Georgia
		Prov("geo").Conn("flo", godip.Coast...).Conn("bet", godip.Sea).Conn("ese", godip.Sea).Conn("pen", godip.Coast...).Conn("app", godip.Land).Conn("neo", godip.Coast...).Conn("gom", godip.Sea).Flag(godip.Coast...).SC(England).
		// South Morocco
		Prov("som").Conn("mis", godip.Land).Conn("nom", godip.Land).Conn("maz", godip.Coast...).Conn("eur", godip.Sea).Conn("cai", godip.Coast...).Conn("wsa", godip.Land).Flag(godip.Coast...).
		// Hispaniola
		...
		// New Orleans
		Prov("neo").Conn("gom", godip.Sea).Conn("geo", godip.Coast...).Conn("app", godip.Land).Conn("stl", godip.Land).Conn("lou", godip.Land).Conn("saf", godip.Coast...).Flag(godip.Coast...).SC(godip.Neutral).
		// Belize
		...
		// Goree
		Prov("gor").Conn("gra", godip.Coast...).Conn("sen", godip.Land).Conn("cav", godip.Coast...).Conn("gre", godip.Sea).Flag(godip.Coast...).SC(France).
		// West Rupert's Land
		...
		// Mexico City
		Prov("mex").Conn("saf", godip.Coast...).Conn("sie", godip.Land).Conn("mer", godip.Coast...).Conn("gom", godip.Sea).Flag(godip.Coast...).SC(Spain).
		// Mazagan
		...
		// Gulf of Lion
		Prov("guo").Conn("tun", godip.Sea).Conn("tun", godip.Sea).Conn("trp", godip.Sea).Conn("ion", godip.Sea).Conn("mss", godip.Sea).Conn("cat", godip.Sea).Conn("bae", godip.Sea).Flag(godip.Sea).
		// New England
		...
		// Irish Sea
		Prov("iri").Conn("sco", godip.Sea).Conn("ote", godip.Sea).Conn("ire", godip.Sea).Conn("ire", godip.Sea).Conn("ote", godip.Sea).Conn("eur", godip.Sea).Conn("eng", godip.Sea).Conn("bri", godip.Sea).Flag(godip.Sea).
		// Castilla y Leon
		Prov("cas").Conn("and", godip.Coast...).Conn("val", godip.Land).Conn("cat", godip.Land).Conn("bor", godip.Coast...).Conn("eur", godip.Sea).Conn("por", godip.Coast...).Flag(godip.Coast...).
		// European Atlantic
		...
		// California
		Prov("cal").Conn("lou", godip.Land).Conn("ore", godip.Land).Conn("sie", godip.Land).Conn("saf", godip.Land).Flag(godip.Land).
		// Sierra Madre
		Prov("sie").Conn("mer", godip.Land).Conn("mex", godip.Land).Conn("saf", godip.Land).Conn("cal", godip.Land).Conn("bei", godip.Land).Flag(godip.Land).
		// Montreal
		Prov("mon").Conn("hud", godip.Sea).Conn("erl", godip.Coast...).Conn("ont", godip.Land).Conn("iro", godip.Coast...).Conn("now", godip.Sea).Conn("now", godip.Sea).Conn("now", godip.Sea).Flag(godip.Coast...).SC(France).
		// Kongo
		Prov("kon").Conn("nam", godip.Coast...).Conn("eaa", godip.Land).Conn("sud", godip.Land).Conn("sla", godip.Coast...).Conn("sea", godip.Sea).Conn("soy", godip.Coast...).Conn("ang", godip.Coast...).Flag(godip.Coast...).
		// Portugal
