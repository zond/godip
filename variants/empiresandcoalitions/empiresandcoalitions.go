...
func EmpiresAndCoalitionsGraph() *graph.Graph {
	return graph.New().
		...
		// Copenhagen
		Prov("cop").Conn("swe", godip.Coast...).Conn("ska", godip.Sea).Conn("sch", godip.Coast...).Conn("bai", godip.Sea).Flag(godip.Coast...).SC(Denmark).
		// Tunisia
		...
		// St. Petersburg
		Prov("stp").Conn("bar", godip.Sea).Conn("fim", godip.Coast...).Conn("fil", godip.Coast...).Conn("gob", godip.Sea).Conn("lio", godip.Coast...).Conn("mos", godip.Land).Conn("ura", godip.Land).Flag(godip.Coast...).SC(Russia).
		// Iceland
		...
		// Hanover
		Prov("han").Conn("wet", godip.Coast...).Conn("bra", godip.Land).Conn("mec", godip.Land).Conn("sch", godip.Coast...).Conn("not", godip.Sea).Flag(godip.Coast...).SC(Britain).
		// Wales
		...
		// Portugal
		Prov("por").Conn("mad", godip.Coast...).Conn("atl", godip.Sea).Conn("and", godip.Coast...).Flag(godip.Coast...).SC(godip.Neutral).
		// Skagerrak
		Prov("ska").Conn("now", godip.Sea).Conn("not", godip.Sea).Conn("sch", godip.Sea).Conn("cop", godip.Sea).Conn("swe", godip.Sea).Conn("chr", godip.Sea).Flag(godip.Sea).
		// Saxony
		...
		// Gibraltar
		Prov("gib").Conn("wem", godip.Sea).Conn("and", godip.Coast...).Conn("atl", godip.Sea).Conn("mor", godip.Coast...).Flag(godip.Coast...).
		// Andalusia
		Prov("and").Conn("val", godip.Coast...).Conn("mad", godip.Coast...).Conn("por", godip.Coast...).Conn("atl", godip.Sea).Conn("gib", godip.Coast...).Conn("wem", godip.Sea).Flag(godip.Coast...).
		...
		// Westphalia
		Prov("wet").Conn("bat", godip.Coast...).Conn("bel", godip.Coast...).Conn("lor", godip.Land).Conn("rhi", godip.Land).Conn("sax", godip.Land).Conn("bra", godip.Land).Conn("han", godip.Coast...).Conn("not", godip.Sea).Flag(godip.Coast...).SC(godip.Neutral).
		// Baltic Sea
		Prov("bai").Conn("sch", godip.Sea).Conn("mec", godip.Sea).Conn("ber", godip.Sea).Conn("kon", godip.Sea).Conn("lio", godip.Sea).Conn("gob", godip.Sea).Conn("swe", godip.Sea).Conn("cop", godip.Sea).Flag(godip.Sea).
		// Arctic Ocean
		Prov("arc").Conn("noa", godip.Sea).Conn("ice", godip.Sea).Conn("now", godip.Sea).Flag(godip.Sea).
		// Piedmont
		Prov("pie").Conn("tys", godip.Sea).Conn("pap", godip.Coast...).Conn("cis", godip.Land).Conn("hel", godip.Land).Conn("lyo", godip.Land).Conn("mar", godip.Coast...).Conn("gol", godip.Sea).Flag(godip.Coast...).SC(godip.Neutral).
		// Budapest
		...
		// Morocco
		Prov("mor").Conn("alg", godip.Coast...).Conn("wem", godip.Sea).Conn("gib", godip.Coast...).Conn("atl", godip.Sea).Flag(godip.Coast...).
		// Belgium
		Prov("bel").Conn("eng", godip.Sea).Conn("brt", godip.Coast...).Conn("par", godip.Land).Conn("lor", godip.Land).Conn("wet", godip.Coast...).Conn("bat", godip.Coast...).Conn("not", godip.Sea).Flag(godip.Coast...).
		// Rhineland
		...
		// Schleswig
		Prov("sch").Conn("bai", godip.Sea).Conn("cop", godip.Coast...).Conn("ska", godip.Sea).Conn("not", godip.Sea).Conn("han", godip.Coast...).Conn("mec", godip.Coast...).Flag(godip.Coast...).
		// Ireland
		Prov("ire").Conn("cel", godip.Sea).Conn("cel", godip.Sea).Conn("noa", godip.Sea).Flag(godip.Coast...).
		// Liverpool
		Prov("lie").Conn("wae", godip.Coast...).Conn("yor", godip.Land).Conn("edi", godip.Coast...).Conn("cel", godip.Sea).Flag(godip.Coast...).SC(godip.Neutral).
		// Adriatic Sea
		Prov("adr").Conn("cis", godip.Sea).Conn("pap", godip.Sea).Conn("apu", godip.Sea).Conn("ion", godip.Sea).Conn("bos", godip.Sea).Conn("cro", godip.Sea).Conn("vie", godip.Sea).Conn("ven", godip.Sea).Flag(godip.Sea).
		// Valencia
		Prov("val").Conn("bae", godip.Sea).Conn("cat", godip.Coast...).Conn("mad", godip.Land).Conn("and", godip.Coast...).Conn("wem", godip.Sea).Flag(godip.Coast...).SC(Spain).
		// English Channel
		...
		// Palermo
		Prov("pal").Conn("tys", godip.Sea).Conn("cen", godip.Sea).Conn("ion", godip.Sea).Flag(godip.Coast...).SC(Sicily).
		// Mecklenburg
		Prov("mec").Conn("bai", godip.Sea).Conn("sch", godip.Coast...).Conn("han", godip.Land).Conn("bra", godip.Land).Conn("ber", godip.Coast...).Flag(godip.Coast...).SC(godip.Neutral).
		// Madrid
		Prov("mad").Conn("nav", godip.Coast...).Conn("atl", godip.Sea).Conn("por", godip.Coast...).Conn("and", godip.Coast...).Conn("val", godip.Land).Conn("cat", godip.Land).Flag(godip.Coast...).SC(Spain).
		// Celtic Sea
		...
		Prov("wem").Conn("bae", godip.Sea).Conn("val", godip.Sea).Conn("and", godip.Sea).Conn("gib", godip.Sea).Conn("mor", godip.Sea).Conn("alg", godip.Sea).Conn("tun", godip.Sea).Conn("tys", godip.Sea).Flag(godip.Sea).
		// Tyrrhenian Sea
		Prov("tys").Conn("pal", godip.Sea).Conn("ion", godip.Sea).Conn("nap", godip.Sea).Conn("pap", godip.Sea).Conn("pie", godip.Sea).Conn("gol", godip.Sea).Conn("bae", godip.Sea).Conn("wem", godip.Sea).Conn("tun", godip.Sea).Conn("cen", godip.Sea).Flag(godip.Sea).
		// Gulf of Bothnia
		Prov("gob").Conn("fil", godip.Sea).Conn("swe", godip.Sea).Conn("bai", godip.Sea).Conn("lio", godip.Sea).Conn("stp", godip.Sea).Flag(godip.Sea).
		// Gascony
		...
		// North Sea
		Prov("not").Conn("lon", godip.Sea).Conn("eng", godip.Sea).Conn("bel", godip.Sea).Conn("bat", godip.Sea).Conn("wet", godip.Sea).Conn("han", godip.Sea).Conn("sch", godip.Sea).Conn("ska", godip.Sea).Conn("now", godip.Sea).Conn("noa", godip.Sea).Conn("edi", godip.Sea).Conn("yor", godip.Sea).Flag(godip.Sea).
		// Urals
		...
		// Papal States
		Prov("pap").Conn("apu", godip.Coast...).Conn("adr", godip.Sea).Conn("cis", godip.Coast...).Conn("pie", godip.Coast...).Conn("tys", godip.Sea).Conn("nap", godip.Coast...).Flag(godip.Coast...).SC(godip.Neutral).
		// Christiania
		...
		// Atlantic
		Prov("atl").Conn("mor", godip.Sea).Conn("gib", godip.Sea).Conn("and", godip.Sea).Conn("por", godip.Sea).Conn("mad", godip.Sea).Conn("nav", godip.Sea).Conn("bay", godip.Sea).Conn("cel", godip.Sea).Conn("noa", godip.Sea).Flag(godip.Sea).
		// Wallachia
		...
		// Finnmark
		Prov("fim").Conn("fil", godip.Land).Conn("stp", godip.Coast...).Conn("bar", godip.Sea).Conn("now", godip.Sea).Conn("chr", godip.Coast...).Conn("swe", godip.Land).Flag(godip.Coast...).
		// Sweden
		...
		// Barents Sea
		Prov("bar").Conn("now", godip.Sea).Conn("fim", godip.Sea).Conn("stp", godip.Sea).Flag(godip.Sea).
		// Kiev
		...
		// Apulia
		Prov("apu").Conn("pap", godip.Coast...).Conn("nap", godip.Coast...).Conn("ion", godip.Sea).Conn("adr", godip.Sea).Flag(godip.Coast...).
		// Breslau
		...
		// Edinburgh
		Prov("edi").Conn("not", godip.Sea).Conn("noa", godip.Sea).Conn("cel", godip.Sea).Conn("lie", godip.Coast...).Conn("yor", godip.Coast...).Flag(godip.Coast...).SC(Britain).
		// Bay of Biscay
		...
		// Napels
		Prov("nap").Conn("ion", godip.Sea).Conn("apu", godip.Coast...).Conn("pap", godip.Coast...).Conn("tys", godip.Sea).Flag(godip.Coast...).SC(Sicily).
		// Gulf of Lyon
		Prov("gol").Conn("cat", godip.Sea).Conn("bae", godip.Sea).Conn("tys", godip.Sea).Conn("pie", godip.Sea).Conn("mar", godip.Sea).Flag(godip.Sea).
		// Finland
		Prov("fil").Conn("stp", godip.Coast...).Conn("fim", godip.Land).Conn("swe", godip.Coast...).Conn("gob", godip.Sea).Flag(godip.Coast...).SC(godip.Neutral).
		// Cisalpine
		Prov("cis").Conn("ven", godip.Coast...).Conn("tyo", godip.Land).Conn("hel", godip.Land).Conn("pie", godip.Land).Conn("pap", godip.Coast...).Conn("adr", godip.Sea).Flag(godip.Coast...).SC(godip.Neutral).
		// Berlin
		...
		// Livonia
		Prov("lio").Conn("pol", godip.Land).Conn("kie", godip.Land).Conn("mos", godip.Land).Conn("stp", godip.Coast...).Conn("gob", godip.Sea).Conn("bai", godip.Sea).Conn("kon", godip.Coast...).Flag(godip.Coast...).
		// Navarre
